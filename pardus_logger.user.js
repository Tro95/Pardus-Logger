...
function saveCombatByServer(isAuto, serverToUse) {
	...
	if (cmbt.additional !== "") {
		var tmp = extractStr(html, "<br><font color=", "</font>");
		tmp = tmp.replace("&quot;", "'");
		tmp = tmp.replace("&quot;", "'");
		cmbt.additional = "<font color=" + tmp + "</font>";
	}
...
/**
 * saveCombatByServer() wrapper
 */
function saveCombat() {
	saveCombatByServer(false, null);
}
...
function saveHack() {
	...
	// New bit for "Permissions"
    function getPermissions() {
        var scripts = document.getElementsByTagName('SCRIPT');
        for (var loop=0; loop<scripts.length; loop++) {
            if (scripts[loop].textContent.indexOf('fillContactsStatic') > -1) {
                var match = scripts[loop].textContent.match(/fillContactsStatic\(\[(.*)\], \$\("#tbl_contacts"\), true/);
                if (match.length == 2) {
                        return '[' + match[1] + ']';
                }
            }
	...
	function prepareHack(hack, account) {
		...
		// any pilot hack
			...
			if (pilotTable.rows.length >= 6)
				hack.turnover = pilotTable.rows[3 + turnoverOffset].cells[0].innerHTML.replace(/,/g, "");
			if (pilotTable.rows.length >= 6)
		...
		if (buildingTable != null) {
			for (var i = 1; i < buildingTable.rows.length; i++) {
                ...
                // New bit for "Permissions"
		hack.permissions = getPermissions();
		hack.foes = [];
		hack.foe_alliances = [];
		var alliance = false;
		if (foeTable != null) {
